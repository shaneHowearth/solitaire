...
func TestStack_Clone(t *testing.T) {
	...
	if originalTop.Rank != Eight {
		t.Errorf("Clone failed: Deep copy was not performed. Modifying clone modified original, got %s.",
			originalTop.Rank.String(),
		)
	}

	if originalStack.Len() != 2 {
		t.Errorf("Clone failed: Adding to clone incorrectly changed original length. Expected 2, got %d",
			originalStack.Len(),
		)
	}
